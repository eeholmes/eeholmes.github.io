...
* Repeat for parameter matrix at time $t$.
    ...
    * Loop over j = i to p.
        ...
        * Compute $I_{ij}(\theta)$ and add to previous time step. This is equation 3.4.69 with the expectation dropped.  Store in `Iij[i,j]` and `Iij[j,i]`. \begin{equation}I_{ij}(\theta) = I_{ij}(\theta) + I_{ij}(\theta)_t\end{equation}
